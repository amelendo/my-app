...
const MapView = ({ track, trackName }: MapViewProps) => {
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      if (saved.length) {
        ...
        map.current.easeTo({ center: [last.lon, last.lat], duration: 0 });
      }
    });
  ...
  savePath(newPath); // persist user path
        ...
        {sportMode && (
          <div className="absolute inset-0 z-20 bg-black/50 flex flex-col justify-between p-6 text-white">
            <div className="text-center space-y-6 mt-6">
              <div className="text-5xl font-bold">{distanceDone.toFixed(2)} km</div>
              <div className="text-3xl">+{Math.round(elevationDone)} m</div>
              <div className="text-3xl">{avgSpeed.toFixed(1)} km/h</div>
            </div>
            ...
            <div className="flex flex-col gap-4 mb-6 items-center">
              ...
              <Button size="sm" variant="secondary" onClick={() => setSportMode(false)}>
                Quitter mode sport
              </Button>
            </div>
          </div>
